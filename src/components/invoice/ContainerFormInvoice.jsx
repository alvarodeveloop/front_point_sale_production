...
function ContainerFormInvoice(props) {
  ...
  const [showSections, setshowSections] = useState( arrayInvoice.includes(props.type) ? 0 : 1) // variable para controlar las pantallas que se muestran
  ...
  const [cotizationData, setCotizationData] = useState(
    Object.assign({},OBJECT_COTIZATION,{
      ...
      type : props.type === "invoiceByGuide" ? 6
      : props.type === "invoiceBySaleNote" ? 5
      : arrayInvoice.includes(props.type) ? 1 
  ...
  const fetchData = (onlyClient = false , isUpdate = false) => {
    ...
    let promises = null
    let isById = arrayById.includes(props.type) && props.match.params.id? true : false; 
    if(!onlyClient){
      promises = [
    ...
    Promise.all(promises).then(result => {
      ...
      if(result.length > 1){
        ...
        if( isById ){
          ...
          if( arrayInvoice.includes(props.type) && props.type !== "invoiceByGuide"){
            let isInvoice = props.type === "facturacion" ? true : false; 
            searchReceptorEmisorInvoice(isInvoice ? false : true, isInvoice ? false : result[indexArray]);
          }else{
            /* logica para algunos datos de las boletas */
            let addressTransmitterArray = Object.assign({},OBJECT_COTIZATION).address_transmitter_array;
            let addressTransmitter = result[indexArray].data.address_transmitter;
            if(arrayBoleta.includes(props.type)){
            ...
            if(arrayGuide.includes(props.type)){
              searchClientReceptorGuide(result[indexArray].data.rut_client,result[indexArray].data);
            }else{
              setCotizationData(oldData => {
                return Object.assign({},oldData,{
                  
                  business_name_transmitter: result[indexArray].data.business_name_transmitter,
                  ...
                  email_transmitter: result[indexArray].data.email_transmitter,
                  phone_transmitter: result[indexArray].data.phone_transmitter,
                  actividad_economica_transmitter: result[indexArray].data.actividad_economica_transmitter,
                  city_transmitter : result[indexArray].data.city_transmitter,
                  comuna_transmitter: result[indexArray].data.comuna_transmitter,
                  type_sale_transmitter: result[indexArray].data.type_sale_transmitter,
                  address_transmitter_array: addressTransmitterArray,
                  
                  comment: result[indexArray].data.comment,
                  date_issue: moment(result[indexArray].data.date_issue).tz('America/Santiago').format('YYYY-MM-DD'),
                  date_issue_invoice: moment().tz('America/Santiago').format('YYYY-MM-DD'),
                  date_expiration: moment(result[indexArray].data.date_expiration).tz('America/Santiago').format('YYYY-MM-DD'),
                  type_api: result[indexArray].data.type_api,
                  discount_global: result[indexArray].data.discount_global,
                  way_of_payment: result[indexArray].data.way_of_payment ? result[indexArray].data.way_of_payment : 1,

                  rut_client: result[indexArray].data.rut_client,
                  business_name_client: result[indexArray].data.business_name_client,
                  address_client: result[indexArray].data.address_client,
                  actividad_economica_client: result[indexArray].data.actividad_economica_client,
                  ...
                  id_guide : props.type === "invoiceByGuide" ? props.match.params.id : oldData.id_guide,
  ...
  const searchReceptorEmisorInvoice = (autoReceptorSearch = false, dataCotizacion = false) => {
    ...
    if(dataCotizacion){
      setCotizationData(currentData => Object.assign({},currentData,{type_invoicing : false,ref: dataCotizacion.data.ref}));
    }
    setshowSections(1);
  }

  const searchClientReceptorGuide = async (rutSearch = false, dataCoti = null) => {
    try {
  ...
  const goToDashboard = () => {
    let route = props.type === "cotizacion" ? '/quotitation/search_quotitation'
                ...
                : arraySaleNote.includes(props.type) ? "/sale_note/sale_note_search" 
                : arrayBoleta.includes(props.type) ? "/bill/bill_search"
                : arrayGuide.includes(props.type) ? "/guide/guide_search" : "/dashboard";  
  ...
  const handleSubmitInvoice = () => {
    ...
    let route =  arrayFacturarInvoice.includes(props.type) ? API_URL+"invoice" 
                : arrayFacturarCotizacion.includes(props.type) ? API_URL+'cotizacion_facturar/'+props.match.params.id 
                : props.type === "guide" ? API_URL+"guide" : "";
    let request =  arrayFacturarInvoice.includes(props.type) ? axios.post(route,object_post) 
                  : arrayFacturarCotizacion.includes(props.type) ? axios.put(route,object_post) 
                  : props.type === "guide" ? axios.post(route,object_post) : "";
    ...
    request.then(result => {
      ...
      if(arrayInvoice.includes(props.type)){
        ...
        toast.success('Proceso completado, espere mientras se genera el documento de la nota')
        setDisplayLoadingModal(false)
        toast.info('Generando el o los pdf, espere por favor...')
        let promises = [];
        result.data.response.forEach((item, i) => {
          promises.push(axios.get(API_URL+'invoice_print/'+item.id+"/"+3+"/2"));
        });
        Promise.all(promises).then(resultPdf => {
          resultPdf.forEach((v,i) => {
            window.open(API_URL+'documents/sale_note/files_pdf/'+v.data.name)
          })
          setTimeout(() => {
            goToDashboard()
          },2000)
  ...
  const onChange = async e => {
    ...
    if(e.target.name === "type_api" || e.target.name === "total_with_iva" || e.target.name === "type_effect" || e.target.name === "type_invoicing"){
      if(e.target.name === "type_invoicing" && arrayInvoice.includes(props.type)){
        if(cotizationData.type_invoicing !== true && cotizationData.type_invoicing !== false){
          ...
          searchReceptorEmisorInvoice( arraySearchDefaultInvoiceRecetor.includes(props.type) ? false : true);
        }else{
    ...
    }else if(e.target.name === "rut_transmitter" || e.target.name === "rut_client" || e.target.name === "rut_driver" || e.target.name === "rut_transfer"){
  ...
  const displayShowSectionsHandler = (section,type,isLastSection = false) => {
    ...
    if(arrayInvoice.includes(props.type) && (cotizationData.type_invoicing !== true && cotizationData.type_invoicing !== false)){
      return false;
    }
    let validate = true;
    ...
    if(isLastSection && ( arrayBoleta.includes(props.type) || arrayGuide.includes(props.type) ) ){
  ...
  const validateSectionHandler = (section) => {
    ...
    // sección 1
    if(section === 1){
      if( arrayInvoice.includes(props.type) || arrayBoleta.includes(props.type) || arrayGuide.includes(props.type) ){
        if(!detailProducts.length){
          return [false, "Debe seleccionar al menos un producto para continuar"];
        }else{
    ...
    //section 3 
    if(section === 3){
      if(arrayInvoice.includes(props.type) || arraySaleNote.includes(props.type)){
        if(!cotizationData.date_issue_invoice || !cotizationData.days_expiration || !cotizationData.way_of_payment){
          return [false, arrayInvoice.includes(props.type) ? "Todos los campos de esta sección son requeridos" : "Todos los campos de esta sección son requeridos menos el descuento global"];
        }else{
          if(arraySaleNote.includes(props.type) && !cotizationData.type_invoicing){
            return [false, "Todos los campos de esta sección son requeridos menos el descuento global"];
          }
          return [true];
        }
      }else if(arrayBoleta.includes(props.type)){
        if(!cotizationData.date_issue || (cotizationData.type_invoicing !== true && cotizationData.type_invoicing !== false)){
    ...
    if(section === 4){
      if(arrayInvoice.includes(props.type)){
        ...
        if( 
          !cotizationData.rut_client || !cotizationData.business_name_client || !cotizationData.address_client || cotizationData.type_buy_client
          || !cotizationData.city_client || !cotizationData.comuna_client || !cotizationData.spin_client || !cotizationData.name_contact
        ){
          return [false, "Los siguientes campos del cliente son requeridos: Rut,razon social,dirección,ciudad,comuna,giro, tipo de compra y nombre del contacto"];
        }
        
        if( 
          !cotizationData.address_transmitter || !cotizationData.city_transmitter || !cotizationData.type_sale_transmitter || cotizationData.actividad_economica_transmitter
          || !cotizationData.type_transfer_trasmitter
        ){
          return [false, "Los siguientes campos del emisor son requeridos: direcció,ciudad,tipo de venta,actividad económica y tipo de traslado"];
        }
        return [true];
      }
    ...
    if(section === 6 && arrayGuide.includes(props.type)){
      if(cotizationData.rut_transfer || cotizationData.patent_transfer || cotizationData.rut_driver || cotizationData.name_driver){
        if(!cotizationData.rut_transfer || !cotizationData.patent_transfer || !cotizationData.rut_driver || !cotizationData.name_driver){
          return [false, "Todos los campos de esta sección son requeridos a menos que no se desee llenar ninguno"];
        }  
      }
    }
  ...
  const displayMembreteCotizacion = () => {
    ...
    let word1 = arraySaleNote.includes(props.type) ? "Nota de Venta"
                : arrayBoleta.includes(props.type) ? "Boleta"
                : arrayGuide.includes(props.type) ? "Guía"
                : arrayInvoice.includes(props.type) ? "Facturación"
                : props.type === "cotizacion" ? "Cotización" : "Orden de Compra"; 
        ...
        <Row className="justify-content-between align-items-center">
            ...
            <Row>
              <Col>
                ...
                <div style={{display: "flex", width: "100%"}}>
                  ...
                  <OverlayTrigger placement={'bottom'} overlay={<Tooltip id="tooltipConfigGastos">
                    Gestione los gastos de la {word2}. No afectará el monto total a facturar pero le dará un estimado entre perdidas y ganancias.
                  </Tooltip>}>
                    <div onClick={()=> displayShowSectionsHandler(2,true)} className={showSections === 2 ? "indicatorsCircleActive" : "indicatorsCircle"}>Gastos</div>
                  </OverlayTrigger>
                  <OverlayTrigger placement={'bottom'} overlay={<Tooltip id="tooltipConfigExtra">
                    Gestione la fechas de inicio y vencimiento además de indicar si el tipo de factura a efectuar desde esta {word2} es de tipo afecta o excenta
                  </Tooltip>}>
            ...
            <Row style={{marginTop: "10px"}}>
              <Col sm={12} md={12} lg={12}>
                ...
                {!arrayCotizacion.includes(props.type) ? (
                    ...
                    <div onClick={()=> displayShowSectionsHandler(5,true)} className={showSections === 5 ? "indicatorsCircleActive" : "indicatorsCircle"}>Referencias</div>
                  </OverlayTrigger>
                  ) : ""}
                  { arrayBoleta.includes(props.type) ? ( 
                    ...
                    <OverlayTrigger placement={'bottom'} overlay={<Tooltip id="tooltipConfigResumen">
                      Seccionar para gestionar datos del transportista de la guía.  
                    </Tooltip>}>
                      <div onClick={()=> displayShowSectionsHandler(6,true)} className={showSections === 6 ? "indicatorsCircleActive" : "indicatorsCircle"}>Transporte</div>
                    </OverlayTrigger>
                  ) : ""}
                  <OverlayTrigger placement={'bottom'} overlay={<Tooltip id="tooltipConfigResumen">
          ...
          {arrayCotizacion.includes(props.type) || arraySaleNote.includes(props.type) ? (
            <Col sm={4} md={4} lg={4}>
              <InputField
                type='text'
                label={(<h5 style={{color: "rgb(153, 31, 31)"}}>Ref.{word1}</h5>)}
                name='id_cotizacion'
                required={true}
                messageErrors={[
                ...
                <Row className="justify-content-center">
                  <Col sm={6} md={6} lg={6}>
                    <Button variant="secondary" size="sm" block={true} onClick={goToDashboard}>Volver a la tabla</Button>
                  </Col>
                </Row>
            </Col>
          ) : (
            ...
            <React.Fragment>
              ...
              { showSections === 0 ? (
                ...
                <>
                  ...
                  <Row className="justify-content-center">
                    <Col sm={4} md={4} lg={4}>
                      <Button variant="secondary" block={true} size="sm" onClick={() => displayShowSectionsHandler(2,false)} type="button">Atrás</Button>
                    </Col>
                    <Col sm={4} md={4} lg={4}>
                      <Button variant="secondary" block={true} size="sm" onClick={() => displayShowSectionsHandler(4,true)} type="button">Siguiente</Button>
                    </Col>
                  </Row>
                </>
              ) : showSections === 4 ? (
                <Row className="justify-content-center">
                  ...
                  <Col sm={4} md={4} lg={4}>
                    <br/>
                    <Button variant="secondary" block={true} size="sm" onClick={() => displayShowSectionsHandler(3,false)} type="button">Atrás</Button>
                  </Col>
                  <Col sm={4} md={4} lg={4}>
                    <br/>
                ...
                <> 
                  ...
                  <Row className="justify-content-center">
                    ...
                    <Col sm={4} md={4} lg={4}>
                      <br/>
                      <Button variant="secondary" block={true} size="sm" onClick={() => arrayBoleta.includes(props.type) || arrayGuide.includes(props.type) ? displayShowSectionsHandler(6,true) : displayShowSectionsHandler(7,true)} type="button">Siguiente</Button>
                    </Col>
                  </Row>
                </>
                ...
                <>
                  <h4 className="text-center title_principal">Datos del transportista</h4>
                  <br/>
                  <TransferComponent
                    cotizationData={cotizationData}
                    onChange={onChange}
                  ...
                  <Row className="justify-content-center">
                    <Col sm={4} md={4} lg={4}>
                      <br/>
                      <Button variant="secondary" block={true} size="sm" onClick={() => displayShowSectionsHandler(5,false)} type="button">Atrás</Button>
                    </Col>
                    <Col sm={4} md={4} lg={4}>
                      <br/>
                      <Button variant="secondary" block={true} size="sm" onClick={() => displayShowSectionsHandler(7,true)} type="button">Siguiente</Button>
                    </Col>
                  </Row>
                </>
              ) : showSections === 7 ?(
                <>
                  ...
                  {arrayCotizacion.includes(props.type) ? (
                    ...
                    <Row className="justify-content-center">
                      <Col sm={3} md={3} lg={3}>
                        <Button variant="secondary" size="sm" block={true} type="button" onClick={()=>displayShowSectionsHandler(6,false,true)}>Atras</Button>
                      </Col>
                      <Col sm={3} md={3} lg={3}>
                        <Button variant="danger" size="sm" block={true} type="button" onClick={handleModalInvoice}>Emitir y Facturar</Button>
                      </Col>
                    </Row>
                  )}
                </>
