                  ...
                  <React.Fragment>
                    {props.bondsPayments.map((v, i) => (
                      <tr key={i}>
                        ...
                        <td><Badge variant="danger" className="font-badge">{[...props.typeBond].find(v1 => v1.id === parseInt(v.id_type_bond)).name}</Badge></td>
                      </tr>
                    ))}
                  </React.Fragment>
                ) : ''}
            ...
            <tbody className="text-center">
              {
                props.cotizationData.total_with_iva ? (
                  <tr>
                    <td>{props.configGeneral.simbolo_moneda}{showPriceWithDecimals(props.configGeneral, displayTotalProduct(detailProducts, discountGlobal, totalWithIva, tax))}</td>
                    <td>{props.configGeneral.simbolo_moneda}{showPriceWithDecimals(props.configGeneral, displayTotalIva(detailProducts, discountGlobal, totalWithIva, tax))}</td>
                    <td>{props.configGeneral.simbolo_moneda}{showPriceWithDecimals(props.configGeneral, displayTotalGastos(gastosDetail))}</td>
                    {props.cotizationData.discount_global ? (
                      <td>{props.configGeneral.simbolo_moneda}{showPriceWithDecimals(props.configGeneral, displayTotalDiscount(detailProducts, discountGlobal, totalWithIva))}</td>
                    ) : ""}
                    <td>{props.configGeneral.simbolo_moneda}{showPriceWithDecimals(props.configGeneral, displayTotalTotal(detailProducts, discountGlobal, totalWithIva, tax, false, gastosDetail))}</td>
                  </tr>
                ) : (
                  <tr>
                    <td>{props.configGeneral.simbolo_moneda}{showPriceWithDecimals(props.configGeneral, displayTotalProduct(detailProducts, discountGlobal, totalWithIva, tax))}</td>
