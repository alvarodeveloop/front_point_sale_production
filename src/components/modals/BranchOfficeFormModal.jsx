...
const BranchOfficeFormModal = (props) => {
  ...
  const onChange = e => {
    if(e.target.name === "is_open"){
      props.setBranchOfficeForm({...props.branchOfficeForm, [e.target.name] : e.target.checked})
    }else{
      props.setBranchOfficeForm({...props.branchOfficeForm, [e.target.name] : e.target.value})
    }
  }

  const onChangeUser = e => {
    if(e.target.name === "rut"){
      props.setUserForm({...props.userForm, [e.target.name] : formatRut(e.target.value)})
    }else{
      props.setUserForm({...props.userForm, [e.target.name] : e.target.value})
    }
  }
  ...
  const onSubmit = e => {
    ...
    if(props.isBranchOffice){
      ...
      if(validateUser.validate){
        ...
        if(modules.length === 0){
          toast.error('Debe seleccionar los módulos del administrador, si después de la creación desea cambiarlos debe ir al módulo de usuarios')
          return false
        }
        dataUser.branch = dataBranch
  ...
  const cleanForm = () => {
    ...
    props.setBranchOfficeForm({
      name: '',
      id: '',
      is_open: true,
    })
    props.setRequiredInput(false)
    setValidated(false)
  }
        ...
        <Modal.Body>
          ...
          <Row className="justify-content-center">
            ...
            {props.isBranchOffice ? (
              <Col sm={4} md={4} lg={4}>
                <Button block={true} variant="danger" size="sm" onClick={props.handleOpenModalAdd} type="button">Cerrar</Button>
              </Col>
            ) : ''}
          </Row>
        </Modal.Body>
...
BranchOfficeFormModal.propTypes = {
  ...
  setBranchOfficeForm: PropTypes.func.isRequired,
