...
export const ProductListDetailPage = (props) => {
  ...
  const downloadExcelTemplate = () => {
    ...
    if(!products.length){
      ...
      toast.info("Modifique solo el precio en la lista del excel y elimine los productos que no desee agregar",{
        autoClose : 7000
      });
      setDisplayLoading(false);
    }
  ...
  const excelRequestHandler = data => {
    axios.post(API_URL+"list_product_excel",{data, id_list : props.match.params.id}).then(result => {
      toast.success("Registros agregados con éxito : "+result.data.positivo+"\n registros no agregados : "+result.data.negativo)
      fetchData();
    }).catch(err => {
          ...
          <>
            {!displayExcelSection ? (
                ...
                <Row>
                  <Col>
                    <Form onSubmit={onSubmitHandler} noValidate validated={validated}>
                      ...
                      <Row>
                        <Col sm={4} md={4} lg={4}>
                          <Button variant="secondary" type="button" block={true} size="sm" onClick={displayExcelSectionHandler}>Agregar por excel <FaFileExcel /></Button>
                        </Col>
                      </Row>
                    </Form>
                  </Col>
              ...
              <Row className="containerExcel justify-content-center align-items-center">
                <Col sm={4} md={4} lg={4}>
                  <Button variant="success" type="button" block={true} size="sm" onClick={downloadExcelTemplate}>Descargar Plantilla <FaFileExcel /></Button>
                </Col>
                <Col sm={4} md={4} lg={4}>
                  <Button variant="success" type="button" block={true} size="sm" onClick={displayInputExcelHandler}>Subir Plantilla <FaFileExcel /></Button>
                </Col>
                <Col sm={4} md={4} lg={4}>
                  <Button variant="secondary" type="button" block={true} size="sm" onClick={displayExcelSectionHandler}>Desplegar tabla</Button>
                </Col>
                <input type="file" id="input-excel" style={{display: "none"}} onChange={addProductExcelHandler} />
              </Row>
            )}
          </>
        )
