...
const BillBondsPage = (props) => {
  ...
  const fetchData = () => {
    ...
    const promise = [
      axios.get(API_URL+'invoice/'+id+"/"+3),
    ]
  ...
  const exportToExcel = () => {
    ...
    if(bonds.length < 1){
      ...
      wb.Props = {
        Title: "Pagos abonados de la boleta "+invoice.ref,
      ...
      FileSaver.saveAs(new Blob([datos],{type:"application/octet-stream"}), 'Abonos de la boleta '+invoice.ref+" fecha: "+moment().tz('America/Santiago').format('DD-MM-YYYY')+'.xlsx')
  ...
  const exportToPdf = () => {
    if(bonds.length > 0){
      setDisplayLoading(true)
      axios.get(API_URL+'print_invoice_bonds/'+props.match.params.id+'/3',{
        responseType: 'blob'
      }).then(result => {
        FileSaver.saveAs(result.data,'Informe de los pagos de la boleta: '+invoice.ref+' fecha: '+moment().tz('America/Santiago').format('DD-MM-YYYY')+".pdf")
        setDisplayLoading(false)
      }).catch(err => {
        setDisplayLoading(false)
        props.tokenExpired(err)
    ...
    <>
      {displayLoading ? (
        ...
        <Container fluid>
          <Row>
            <Col sm={7} md={7} lg={7}>
              <h4 className="title_principal">Historial de Pagos de la Boleta {invoice  ? (<Badge variant="danger" className="font-badge">{invoice.folio_bill}</Badge>) : ''}</h4>
            </Col>
            <Col sm={5} md={5} lg={5}>
              <Button variant="primary" block={true} type="button" onClick={goToInvoice} size="sm">Volver a las Boletas</Button>
            </Col>
          </Row>
...
export default connect(mapStateToProps,{})(BillBondsPage)
