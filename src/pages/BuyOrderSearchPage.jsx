      ...
      if(status >= 1 && status <= 2){
        ...
        if(status === 3){
          ...
          if(validate){
            ...
            toast.error("La orden de compra no ha podido ser anulada debido a que 1 o más facturas no fueron anuladas")
          }
        }else{
          if(Object.keys(cotizationAction).length){
            setCotizationAction({...cotizationAction, status: 2})
    ...
    <Container fluid>
      <Row>
        <Col sm={6} md={6} lg={6} className="text-center">
          ...
          <Button block={true} variant="success" onClick={goToForm} size="sm">Nueva orden  <FaPlusCircle /></Button>
        </Col>
