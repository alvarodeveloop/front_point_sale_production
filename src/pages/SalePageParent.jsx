...
const SalePageParent = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.configGeneral.is_syncronized) {
      ...
      toast.error(
        "Debe sincronizar su cuenta en la configuración general para utilizar este módulo"
      );
      setTimeout(() => {
    ...
    Promise.all(promises)
      .then((result) => {
        if (result[1].data && result[0].data) {
          ...
          if (promises.length > 2) {
            if (result[2].data.cashOpen) {
              ...
              setCashRegisterUser({
                ...
                amount_cash_default:
                  result[2].data.cash_user.amount_cash_default,
          ...
          if (!result[1].data) {
            toast.error(
              "Error la empresa no ha hecho la configuración general"
            );
            setTimeout(function () {
              props.history.replace("/config/config_general");
            }, 2000);
            ...
            toast.error(
              "Error, debe hacer su configuración de la tienda primero"
            );
            setTimeout(function () {
  ...
  const calculateTotalProducts = () => {
    let total = 0;
    props.sale.rooms[props.sale.idCartSelected].carts.registered.forEach(
      (v, i) => {
        total += parseInt(v.cantidad, 10);
      }
    );

    props.sale.rooms[props.sale.idCartSelected].carts.not_registered.forEach(
      (v, i) => {
        total += parseInt(v.cantidad, 10);
      }
    );
        ...
        <Row className="">
          ...
          <Col sm={6} md={6} lg={6} xs={6}>
            ...
            {showPriceWithDecimals(
              props.configGeneral,
              props.sale.rooms[props.sale.idCartSelected].totales.total
            )}
          </Col>
        </Row>
        <br />
  ...
  const handleChoiceCashRegister = (cashData) => {
    setCashRegisterUser({
      ...
      amount_cash_default: cashData.amount_cash_default
        ? cashData.amount_cash_default
        : 0,
  ...
  const onSubmit = (e) => {
    ...
    axios
      .post(API_URL + "cash_register_open_close", data)
      .then((result) => {
                ...
                <Row>
                  <Col sm={12} md={12} lg={12}>
                    <h4 className="title_principal">
                      Establezca el saldo inicial de la Caja
                    </h4>
                  </Col>
                </Row>
                <hr />
                <Row className="justify-content-center">
                  <Col sm={6} md={6} lg={6} className="text-center">
                    <h5 style={{ color: "rgb(180, 55, 33)" }}>
                      CAJA N° {cashRegisterUser.nro_caja}
                    </h5>
                    <Image
                      src={require("../assets/img/caja_registradora.jpg")}
                      style={{ width: "50%" }}
                    />
                    <br />
            ...
            <Row>
              <Col sm={12} md={12} lg={12}>
                <Row className="justify-content-center">
                  <Col sm={12} md={12} lg={12}>
                    <h4 className="title_principal">
                      Escoja la Caja con la que trabajará
                    </h4>
                  </Col>
                </Row>
                <hr />
                <Row className="justify-content-center">
                  {cashRegisters.length > 0 ? (
                      ...
                      {cashRegisters.map((v, i) => (
                        ...
                        >
                          ...
                          <Button
                            ...
                            block={true}
                            size="sm"
                            onClick={() => handleChoiceCashRegister(v)}
                          >
                            Seleccionar Caja
                          </Button>
                        </Col>
                    ...
                    <Col sm={6} md={6} lg={6} className="text-center">
                      ...
                      <br />
                      <h4 className="">
                        No posee cajas registradoras asociadas
                      </h4>
                      <Button
                        variant="success"
                        type="button"
                        block={true}
                        size="sm"
                        onClick={goToCashRegister}
                      >
                        Agregar Caja Registradora <FaPlusCircle />
                      </Button>
                    </Col>
                  )}
                </Row>
