...
const InvoicePrintPage = (props) => {
  ...
  const calculateIva = (product) => {
    if (product.is_neto || product.is_neto === undefined) {
      ...
      let result =
        (product.price * product.quantity * props.configStore.tax) / 100;
      iva_total += result;
  ...
  if (sale) {
      ...
      <Container>
        <Row className="justify-content-center">
          <Col sm={4} md={4} lg={4} xs={12} className="text-center">
            <Image
              src={API_URL + "images/store/logo/" + props.configStore.logo}
              style={{ width: "40%" }}
            />
          </Col>
        </Row>
        ...
        <Row className="justify-content-center">
          <Col sm={6} md={6} lg={6} xs={6} className="div_client">
            <Row>
              ...
              <Col sm={6} md={6} lg={6} xs={6}>
                {sale.client ? <h6>{sale.client.name_client}</h6> : ""}
              </Col>
            </Row>
            <Row>
              <Col sm={6} md={6} lg={6} xs={6}>
            ...
            <Row>
              ...
              <Col sm={6} md={6} lg={6} xs={6}>
                {sale.client ? <h6>{sale.client.address}</h6> : ""}
              </Col>
            </Row>
            <Row>
              <Col sm={6} md={6} lg={6} xs={6}>
              ...
              <Col sm={6} md={6} lg={6} xs={6}>
                {sale.client ? <h6>{sale.client.phone}</h6> : ""}
              </Col>
            </Row>
          </Col>
        </Row>
                ...
                {sale.products.map((v, i) => {
                  ...
                  return (
                    <tr key={i}>
                      ...
                      <td>
                        {props.config.simbolo_moneda}
                        {showPriceWithDecimals(props.config, v.price)}
                      </td>
                      <td>{v.quantity}</td>
                      <td>
                        {v.product.is_neto ? "" : props.config.simbolo_moneda}
                        {iva_value}
                      </td>
                      <td>
                        {props.config.simbolo_moneda}
          ...
          <Col sm={6} md={6} lg={6} xs={6} className="div_client">
            <table className="rowTable" border="1">
              <tbody>
                ...
                <tr>
                  <td>
                    El acuse de recibo que se declara en esteacto, de acuerdo a
                    lo dispuesto en la letra b)del artículo 4º y la letra c) del
                    artículo 5º de laley 19.983, acredita que la entrega
                    demercadería(s) o servicio(s) prestado(s) ha(n)sido
                    recibo(s)
                  </td>
                </tr>
              </tbody>
    ...
    return <h1>Sin datos de factura</h1>;
  }
};
...
InvoicePrintPage.defaultProps = {
  config: JSON.parse(sessionStorage.getItem("configGeneral")),
